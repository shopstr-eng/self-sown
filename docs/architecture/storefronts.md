...
- **Seller flow** (`components/settings/custom-domain-section.tsx`, mounted inside `shop-profile-form.tsx`): submit domain → receive TXT (`_self-sown.<domain>`) + CNAME (subdomain) or A-record (apex, real IPs of the canonical platform host resolved live + cached 5 min, overridable via `CUSTOM_DOMAIN_APEX_IPS`) instructions → "Check DNS" → status badge.
...
- **Env**: `ADMIN_PUBKEYS` (comma-separated lowercase hex), `DOMAINS_ADMIN_EMAIL` (optional; falls back to the SendGrid sender address), `REPLIT_DEPLOYMENT_HOST` (default `milk-market.replit.app`), `CUSTOM_DOMAIN_APEX_IPS` (optional pinned IPs), `ADMIN_API_SECRET` (optional bearer for server-to-server admin reads).
