...
- The tool surface is a curated allowlist (`utils/assistant/tools.ts`) — no fund movement, no deletes of listings/discounts/flows, no direct buyer messaging, no decrypted DM content, and no billing/relay config. Order-status/shipping tools still send templated buyer notifications. One-off broadcast emails and Shippo label purchases ARE allowed: broadcasts fail closed on the seller's verified sender domain with a per-recipient idempotency ledger and a daily cap, and labels bill to the seller's own connected Shippo account behind an atomic one-label-per-order claim. Enforced both when exposing tools to the model and again at call time.
