...
- `Invoice` has NO top-level `payment_intent` — resolve via `stripe.invoicePayments.list({ invoice })` → `payment.payment_intent`, then `paymentIntents.retrieve` for the client_secret. `expand: ["latest_invoice.payment_intent"]` on subscription create silently yields nothing. FIXED for `create-cart-subscription` via the shared dual-shape helper `resolveSubscriptionPaymentIntent` (utils/stripe/subscription-payment-intent.ts); the sibling routes `create-subscription.ts` and `pro/create-subscription.ts` still read the removed field.
