...
- `Invoice` has NO top-level `payment_intent` — resolve via `stripe.invoicePayments.list({ invoice })` → `payment.payment_intent`. `expand: ["latest_invoice.payment_intent"]` on subscription create silently yields nothing, so `create-cart-subscription` returns `clientSecret: null` (buyer card form is gated on it in cart-invoice-card.tsx).
