...
jest.mock("@/components/storefront/product-page-renderer", () => ({
  __esModule: true,
  default: () => <div data-testid="product-page-renderer" />,
}));

jest.mock("@/components/storefront/formatted-text", () => ({
  __esModule: true,
  default: () => <div data-testid="formatted-text" />,
}));
...
describe("Listing page direct-load reconciliation", () => {
  ...
  beforeEach(() => {
    ...
    routerState = {
      ...
      replace: jest.fn(),
      back: jest.fn(),
      asPath: `/listing/${relayHintedIdentifier}`,
  ...
  test("clears stale listing state when the route changes to a different listing", async () => {
    ...
    routerState = {
      ...routerState,
      asPath: "/listing/different-listing-route",
