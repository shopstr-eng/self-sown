...
// uuid v14 ships ESM-only and is pulled in transitively via the real
// nostr-manager/nip46 signer import chain; mock it so Jest doesn't try to
// parse the untransformed ESM module. Not exercised by these tests.
jest.mock("uuid", () => ({
  v4: () => "mock-uuid-1234",
}));

// nostr-tools/nip49 transitively pulls in @scure/base (ESM-only) via the real
// nsec signer import chain; mock it to avoid the untransformed ESM module.
// Only used for ncryptsec handling, which these tests don't exercise.
...
describe("ZapsnagButton Component", () => {
  ...
  test("handles error if seller has no LUD16", async () => {
    ...
    expect(window.alert).toHaveBeenCalledWith(
      expect.stringContaining("Vendor has not set up a Lightning Address")
    );
  });
